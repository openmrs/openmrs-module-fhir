...
public class PractitionerStrategy implements GenericPractitionerStrategy {
	...
	public Practitioner createFHIRPractitioner(Practitioner practitioner) {
		...
		Person personFromRequest = FHIRPractitionerUtil
				.extractOpenMRSPerson(practitioner); // extracts openmrs person from the practitioner representation
		...
		if (personFromRequest
				== null) { // if this is true, that means the request doesn't have enough attributes to create a person from it, or attach a person from existing ones
			List<HumanName> humanNames = practitioner.getName();
			if (humanNames
					!= null) { // check whether atleast one name is exist. if so we can create a practitioner without attaching a person, just with a name.
				for (HumanName humanName : humanNames) {
					...
					for (StringType givenName : givenNames) {
						practionerName =
								practionerName + " " + valueOf(givenName.getValue()); // will create a name like "John David"
					}
					if ("".equals(
							practionerName)) { // there is no given name or family name. cannot proceed with the request
		...
		if (personFromRequest
				!= null) { // if this is not null, we can have a person resource along the practitioner resource
			Person personToProvider = FHIRPractitionerUtil.generateOpenMRSPerson(
					personFromRequest); // either map to an existing person, or create a new person for the given representation
